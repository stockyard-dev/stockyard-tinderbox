...
async function load(){var r=await fetch(A+'/experiments').then(function(r){return r.json()});items=r.experiments||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.hypothesis||'').toLowerCase().includes(q)||(i.variant_a||'').toLowerCase().includes(q)||(i.variant_b||'').toLowerCase().includes(q)});
...
if(i.hypothesis)h+='<div class="item-sub">'+esc(i.hypothesis)+'</div>';
...
if(i.variant_a)h+='<span>'+esc(i.variant_a)+'</span>';
if(i.variant_b)h+='<span>'+esc(i.variant_b)+'</span>';
if(i.traffic_pct)h+='<span>'+esc(i.traffic_pct)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.winner)h+='<span>'+esc(i.winner)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/experiments/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",hypothesis:"",variant_a:"",variant_b:"",traffic_pct:"",status:"",winner:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' TINDERBOX</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Hypothesis</label><input id="f-hypothesis" value="'+esc(i.hypothesis)+'"></div>';
h+='<div class="fr"><label>Variant A</label><input id="f-variant-a" value="'+esc(i.variant_a)+'"></div>';
h+='<div class="fr"><label>Variant B</label><input id="f-variant-b" value="'+esc(i.variant_b)+'"></div>';
h+='<div class="fr"><label>Traffic Pct</label><input id="f-traffic-pct" value="'+esc(i.traffic_pct)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Winner Variant</label><input id="f-winner" value="'+esc(i.winner)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),hypothesis:document.getElementById('f-hypothesis').value.trim(),variant_a:document.getElementById('f-variant-a').value.trim(),variant_b:document.getElementById('f-variant-b').value.trim(),traffic_pct:document.getElementById('f-traffic-pct').value.trim(),status:document.getElementById('f-status').value.trim(),winner:document.getElementById('f-winner').value.trim()};
if(editId){await fetch(A+'/experiments/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/experiments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
