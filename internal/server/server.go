package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-tinderbox/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-tinderbox"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/pipelines",s.handleList)
    s.mux.HandleFunc("POST /api/pipelines",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/pipelines/{id}",s.handleDelete)
    s.mux.HandleFunc("POST /api/pipelines/{id}/trigger",s.handleTrigger)
    s.mux.HandleFunc("GET /api/pipelines/{id}/runs",s.handleRuns)
